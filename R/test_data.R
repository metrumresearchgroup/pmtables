...
# id <- pmplots_data_id()
# spec <- ys_help$spec()
# ys_get_unit <- function(spec) map(spec, ~ yspec:::unit(.x))
# ys_get_short <- function(spec) map(spec, "short")
#
#
# # re-decode -----
# re <- yaml::yaml.load_file("recode.yml")
#
...
#
# mf <- c("male", "female")
# id <- mutate(id, SEX = mf[(ID%% 2)+1])
# cols <- "CPc,RF,SEX,STUDYc" %>% cvec_cs
# id <- yspec_add_factors(id,spec2,CP,RF)
# id <- mutate(id, PEDIATRIC = factor(AGE > 30, labels=c("pediatric", "adult")))
#
...
# data.cat <- select(id, ID,STUDY,SEX,RF_f,CP_f,PEDIATRIC)
#
