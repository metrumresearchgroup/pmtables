...
Test IDs go at the end of the `test_that()` description in brackets, e.g.
`[PMT-TEST-0248]`. The dominant prefix is `PMT-TEST`, but newer areas use topical
prefixes (`PMT-INVEN`, `PMT-STFUN`, `PMT-UTIL`). IDs are referenced from
`inst/validation/pmtables-stories.yaml` (mrgvalprep format), which maps user stories
to tests for the validation docs built by `inst/validation/build-validation-docs.R`.
When adding a user-visible feature, add the story and wire the new test ID to it.
