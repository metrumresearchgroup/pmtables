...
```{r, results = "asis"}
out <- stable(pmt.summ, col_bold = TRUE)
...
```{r, results = "asis"}
out <- stable(pmt.summ, r_file = "validate.Rmd", output_file = "file-names.tex")
...
```{r, results = "asis"}
out <- stable(pmt.summ, notes = c("WT: weight", "ALB: albumin"))
...
out <- stable(pmt.summ, notes = c("WT: weight", "ALB: albumin"), note_config = conf)
...
```{r, results = "asis"}
out <- stable(pmt.summ, panel = "STUDY")
...
```{r, results = "asis"}
out <- stable(pmt.summ, panel = as.panel("STUDY", prefix = "Study: "))
...
```{r, results = "asis"}
out <- stable(pmt.summ, clear_reps = "STUDY")
...
```{r, results = "asis"}
out <- stable(pmt.summ, clear_grouped_reps = "STUDY,DOSE")
...
```{r, results = "asis"}
out <- stable(pmt.summ, hline_at = c(2,4,6))
...
```{r, results = "asis"}
out <- stable(pmt.summ, hline_from = "STUDY")
...
```{r, results = "asis"}
out <- stable(pmt.summ, hline_from = "STUDY", clear_reps = "STUDY")
...
out <- stable(pmt.summ, align = align)
...
```{r, results = "asis"}
out <- stable(pmt.summ, col_rename = c(Weight = "WT", Dose = "DOSE"))
...
```{r, results = "asis"}
out <- stable(pmt.summ, col_blank = "WT,ALB")
...
```{r, results = "asis"}
out <- stable(pmt.summ, col_rename = c("Protocol...Number" = "STUDY", Weight = "WT"))
...
```{r, results = 'asis'}
out <- pt_cont_long(
  pmt.first, cols = "WT,CRCL,ALB", panel = "STUDYf", 
  units = units
) %>% as_stable()
...
```{r}
if(params$save) {
  stable_save(out, file = "inventory-by.tex")   
}
```
...
```{r}
if(params$save) {
  stable_save(out, file = "inventory-panel-by.tex")   
}
```
