...
```{r, results = "asis"}
out <- stable(pmt_summarized, col_bold = TRUE)
...
```{r, results = "asis"}
out <- stable(pmt_summarized, col_rename = c(Weight = "WT", Dose = "DOSE"))
...
```{r}
if(params$save) {
  stable_save(out, file = "col-rename.tex")   
}
```
...
```{r, results = "asis"}
out <- stable(pmt_summarized, col_blank = "WT,ALB")
...
```{r, results = "asis"}
out <- stable(pmt_summarized, col_rename = c("Protocol...Number" = "STUDY", Weight = "WT"))
